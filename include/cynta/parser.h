...
#include <stdlib.h>
    ...
    typedef enum cynta_parser_error_t
    {
        ...
        CYNTA_PARSER_ERROR_MEMORY_ALLOCATION,
...
#define CYNTA_PARSER_ERROR(err) ((err) == CYNTA_PARSER_SUCCESS                   ? "CYNTA_PARSER_SUCCESS"                 \
                                 ...
                                 : (err) == CYNTA_PARSER_ERROR_MEMORY_ALLOCATION ? "CYNTA_PARSER_ERROR_MEMORY_ALLOCATION" \
    ...
    {
        cynta_parser_error_t (*apply)(struct cynta_parser_t *, cynta_stream_t *, void **, size_t *);
        void (*delete)(struct cynta_parser_t *);
    } cynta_parser_t;

    static inline cynta_parser_error_t cynta_parser_apply(cynta_parser_t *parser, cynta_stream_t *stream, void **out, size_t *out_size)
    {
        return parser == NULL
                   ? CYNTA_PARSER_ERROR_NULL_POINTER
                   : parser->apply(parser, stream, out, out_size);
    }
