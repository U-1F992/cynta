...
static void test_stream(void) {
    ...
    assert(CYNTA_STREAM_SUCCESS ==
           test_array_stream_init(&stream, origin, origin_size));
...
static bool test_parser_helper(size_t index, cynta_parser_t *parser,
    ...
    assert(CYNTA_STREAM_SUCCESS ==
           test_array_stream_init(&test_stream, stream, stream_size));

    cynta_uint8_array_t actual_out;
    cynta_parser_error_t actual = cynta_parser_apply(
        parser, (cynta_stream_t *)&test_stream, (void *)&actual_out);
    if (expected != actual) {
        fprintf(stderr, "%sindex: %d, expected: %s, actual: %s%s\n",
                TEST_TEXT_RED, index, CYNTA_PARSER_ERROR(expected),
                CYNTA_PARSER_ERROR(actual), TEST_TEXT_RESET);
        return false;
    }
    ...
    if (expected_out_size != actual_out.size) {
        fprintf(stderr,
                ...
                TEST_TEXT_RED, index, expected_out_size, actual_out.size,
                TEST_TEXT_RESET);
        return false;
    }
    ...
    for (size_t i = 0; i < actual_out.size; i++) {
        if (expected_out[i] != actual_out.data[i]) {
            fprintf(stderr,
                    ...
                    TEST_TEXT_RED, index, i, expected_out[i], i,
                    actual_out.data[i], TEST_TEXT_RESET);
            failure = true;
        }
        if (failure) {
            return false;
...
static size_t test_any(void) {
    ...
    for (size_t i = 0; i < test_cases_size; i++) {
        ...
        if (!test_parser_helper(i, test_case->parser, test_case->stream,
                                test_case->stream_size, test_case->expected,
...
static size_t test_sequence(void) {
    ...
    test_case_t test_cases[] = {
        {.parser = cynta_sequence(2, cynta_value(127), cynta_value(128)),
         .stream = (uint8_t[]){127, 128},
         .stream_size = 2,
         .expected = CYNTA_PARSER_SUCCESS,
         .expected_out = (uint8_t[]){127, 128},
         .expected_out_size = 2},
        {.parser = cynta_sequence(2, cynta_any(), cynta_value(128)),
         .stream = (uint8_t[]){127, 128},
         .stream_size = 2,
         .expected = CYNTA_PARSER_SUCCESS,
         .expected_out = (uint8_t[]){127, 128},
...
static size_t test_choice(void) {
    ...
    test_case_t test_cases[] = {
        {.parser = cynta_choice(3, cynta_value(0xAB), cynta_value(0xCD),
                                cynta_value(0xEF)),
         .stream = (uint8_t[]){0xAB},
         .stream_size = 1,
         .expected = CYNTA_PARSER_SUCCESS,
...
static size_t test_complex_examples(void) {
    ...
    test_case_t test_cases[] = {
        {.parser =
             cynta_sequence(3, cynta_value(0xAB), cynta_many(cynta_value(128)),
                            cynta_value(129)),
        ...
        {.parser = cynta_choice(
             2,
             cynta_choice(
                 2, // ORCA GC Controller
                 cynta_value((uint8_t)'@'),
                 cynta_sequence(5, cynta_value(0x80),
                                cynta_many(cynta_value(0xFF)), cynta_any(),
                                cynta_many(cynta_value(0xFF)), cynta_any())),
             cynta_sequence(2, // NX Macro Controller
                            cynta_value(0xAB), cynta_repeat(cynta_any(), 10))),
         .stream = (uint8_t[]){0xAB, 0xFF, 0x3F, 0x08, 0x80, 0x80, 0x80, 0x80,
                               0x00, 0x00, 0x00},
         .stream_size = 11,
         .expected = CYNTA_PARSER_SUCCESS,
         .expected_out = (uint8_t[]){0xAB, 0xFF, 0x3F, 0x08, 0x80, 0x80, 0x80,
                                     0x80, 0x00, 0x00, 0x00},
...
int main(void) {
    ...
    if (test_failure_count == 0) {
        ...
        fprintf(stderr, "%s%d errors found%s\n", TEST_TEXT_RED,
                test_failure_count, TEST_TEXT_RESET);
        return 1;
    }
}